...
const adminController = {
  ...
  usersPage: (req, res) => {
    ...
    User.findAll({ include: [{ model: Tweet }, { model: User, as: 'Followings' }, { model: User, as: 'Followers' }, { model: Tweet, as: 'userTweetsLikes' }] }).then(users => {
      console.log(users[0].Tweets)
      //處理description | 僅顯示50字元
      //map + operator + 修改description屬性
      users = users.map(user => ({
        ...user.dataValues,
        TweetsAcount: user.Tweets.length,
        FollowingsAcount: user.Followings.length,
        FollowersAcount: user.Followers.length,
        LikesAcount: user.userTweetsLikes.length,
      }));
